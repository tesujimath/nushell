...
fn setup_fake_config(playground: &mut Playground) -> PathBuf {
    ...
    make_absolute_and_clean_with(&path, playground.cwd()).unwrap_or(path)
}
...
fn test_alternate_config_path() {
    ...
    let config_path = nu_path::make_absolute_and_clean_with(config_file, &cwd)
    ...
    let env_path =
        nu_path::make_absolute_and_clean_with(env_file, &cwd).expect("Could not get env path");
    let actual = nu!(
        cwd: &cwd,

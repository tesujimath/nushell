...
pub fn find_in_dirs(
    ...
    pub fn find_in_dirs_with_id(
        ...
        if let Ok(p) = make_absolute_with(filename, actual_cwd) {
        ...
        dirs_var_name
            ...
            .map(|lib_dir| -> Option<PathBuf> {
                ...
                let dir_abs = make_absolute_with(dir, actual_cwd).ok()?;
                    ...
                    if let Ok(dirs) = lib_dirs.as_list() {
                        for lib_dir in dirs {
                            if let Ok(dir) = lib_dir.to_path() {
                                ...
                                if let Ok(dir_abs) = make_absolute_with(dir, actual_cwd) {
                                    if let Ok(path) = make_absolute_with(filename, dir_abs) {
                                        return Some(path);
                                    }
                                }
                            }

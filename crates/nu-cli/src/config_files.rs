...
pub fn add_plugin_file(
    ...
    if let Ok(cwd) = engine_state.cwd_as_string(None) {
        if let Some(plugin_file) = plugin_file {
            ...
            if let Ok(path_dir) = make_absolute_with(&path_dir, &cwd) {
                ...
                let path = make_absolute_with(&path, &cwd).unwrap_or(path);
            ...
            let mut plugin_path = make_absolute_with(&plugin_path, &cwd).unwrap_or(plugin_path);
            plugin_path.push(PLUGIN_FILE);
            let plugin_path = make_absolute_with(&plugin_path, &cwd).unwrap_or(plugin_path);
...
pub fn migrate_old_plugin_file(engine_state: &EngineState, storage_path: &str) -> bool {
    ...
    let Some(config_dir) = nu_path::config_dir().and_then(|mut dir| {
        ...
        nu_path::make_absolute_with(dir, &cwd).ok()
    }) else {
        return false;
    };

    let Ok(old_plugin_file_path) = nu_path::make_absolute_with(OLD_PLUGIN_FILE, &config_dir) else {
        return false;
    };

...
pub fn nu_run_test(opts: NuOpts, commands: impl AsRef<str>, with_std: bool) -> Outcome {
    ...
    let test_bins = nu_path::make_absolute_with(&test_bins, cwd).unwrap_or_else(|e| {
        panic!(
...
{
    let test_bins = crate::fs::binaries();
    let test_bins = nu_path::make_absolute_with(&test_bins, ".").unwrap_or_else(|e| {
        panic!(
            "Couldn't canonicalize dummy binaries path {}: {:?}",
            test_bins.display(),
            e
    ...
    let plugin_paths_quoted: Vec<String> = plugins
        ...
        .map(|plugin_name| {
            ...
            let plugin_path = nu_path::make_absolute_with(&plugin, &test_bins)

...
pub(crate) fn read_config_file(
    ...
    if let Some(file) = config_file {
        ...
        match engine_state.cwd_as_string(Some(stack)) {
            Ok(cwd) => {
                if let Ok(path) = make_absolute_with(&file.item, cwd) {
...
pub(crate) fn set_config_path(
    ...
    let config_path = match config_file {
        Some(s) => make_absolute_with(&s.item, cwd).ok(),
        None => nu_path::config_dir().map(|mut p| {
            p.push(NUSHELL_FOLDER);
            let mut p = make_absolute_with(&p, cwd).unwrap_or(p);
            p.push(default_config_name);
            make_absolute_with(&p, cwd).unwrap_or(p)
        }),
    };

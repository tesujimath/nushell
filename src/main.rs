...
fn main() -> Result<()> {
    ...
    if let Ok(xdg_config_home) = std::env::var("XDG_CONFIG_HOME") {
        if !xdg_config_home.is_empty() {
            if nushell_config_path
                != make_absolute_and_clean_with(&xdg_config_home, &init_cwd)
                    ...
                    .join("nushell")
            {
                ...
                nu_path::get_absolute_clean_path(dirs_next::config_dir()).map(|p| p.join("nushell"))
            {
    ...
    if let Some(plugins) = &parsed_nu_cli_args.plugins {
        ...
        for plugin_filename in plugins {
            ...
            let filename = make_absolute_and_clean_with(&plugin_filename.item, &init_cwd)
